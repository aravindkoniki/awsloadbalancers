...
resource "aws_lb" "load_balancer" {
  ...
  tags = merge(
    ...
    var.lb_tags,
    {
      Name = upper(var.name != null ? var.name : var.name_prefix)
    },
    {
      "ManagedBy" = "Terraform"
    }
...
resource "aws_lb_target_group" "lb_target_group" {
  ...
  connection_termination             = var.load_balancer_type != "gateway" ? lookup(var.target_groups[count.index], "connection_termination", null) : null
  ...
  tags = var.load_balancer_type != "gateway" ? merge(
    var.tags,
    var.target_group_tags,
    lookup(var.target_groups[count.index], "tags", {}),
    {
      "Name" = upper(lookup(var.target_groups[count.index], "name", lookup(var.target_groups[count.index], "name_prefix", "")))
    },
    {
      "ManagedBy" = "Terraform"
    }
...
resource "aws_lb_listener_rule" "https_listener_rule" {
  ...
  dynamic "action" {
    ...
    content {
      type             = action.value["type"]
      target_group_arn = aws_lb_target_group.lb_target_group[lookup(action.value, "target_group_index", count.index)].id
    }
  }
    ...
    content {
      ...
      forward {
        dynamic "target_group" {
          ...
          content {
            arn    = aws_lb_target_group.lb_target_group[target_group.value["target_group_index"]].id
...
resource "aws_lb_listener" "frontend_http_tcp" {
  ...
  port     = var.load_balancer_type != "gateway" ? var.http_tcp_listeners[count.index]["port"] : null
  protocol = var.load_balancer_type != "gateway" ? var.http_tcp_listeners[count.index]["protocol"] : null
...
resource "aws_lb_listener" "frontend_https" {
  ...
  port            = var.load_balancer_type != "gateway" ? var.https_listeners[count.index]["port"] : null
  protocol        = var.load_balancer_type != "gateway" ? lookup(var.https_listeners[count.index], "protocol", "HTTPS") : null
  ...
  dynamic "default_action" {
    ...
    content {
      type             = "forward"
      target_group_arn = aws_lb_target_group.lb_target_group[lookup(default_action.value, "target_group_index", count.index)].id
    }
  }
